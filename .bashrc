...
if [[ $FAST_STARTUP == 0 ]]; then
  ...
  # defer other non-essential sourcing
  test -e "${HOME}/.iterm2_shell_integration.bash" && source "${HOME}/.iterm2_shell_integration.bash"

else

  _dbg "Skipping $HOME/.extra and fzf loading for speed"
  _dbg "Skipping loading iterm2 components"
fi

# Final must-load components:
export NVM_DIR=$HOME/.nvm
source_and_log "$NVM_DIR/nvm.sh"
